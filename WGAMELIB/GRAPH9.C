// M A C R O S ////////////////////////////////////////////////////////////////
//from C700 include files (so I can compile with watcom)
#define C700_FP_SEG(fp) (*((unsigned __far *)&(fp)+1))
#define C700_FP_OFF(fp) (*((unsigned __far *)&(fp)))
...
_dos_freemem(C700_FP_SEG(driver_ptr));
...
segm = C700_FP_SEG(addr);
offm = C700_FP_OFF(addr) + header_length;
...
segm = C700_FP_SEG(status);
offm = C700_FP_OFF(status);
...
C700_FP_SEG(driver_ptr) = segment;
C700_FP_OFF(driver_ptr) = 0;
...
C700_FP_SEG(data_ptr) = segment;
C700_FP_OFF(data_ptr) = 0;
...
// make sure it's a voc file
   ...
   if ((data_ptr[0] != 'C') || (data_ptr[1] != 'r'))
      ...
      _dos_freemem(C700_FP_SEG(data_ptr));
      return(0);
...
_dos_freemem(C700_FP_SEG(sound_ptr));
